...
func UlNasTransport3(ue *context.UEContext, requestType uint8) ([]byte, error) {
	log.Info("UL NAS Transport 3")
	pdu := getUlNasTransport_PduSessionReleaseComplete(ue.PduSession.Id, requestType, ue.PduSession.Dnn, &ue.PduSession.Snssai)
	if pdu == nil {
		return nil, fmt.Errorf("Error encoding %s IMSI UE PduSession Release Request Msg", ue.UeSecurity.Supi)
	}
	log.Info("UL NAS Transport 3 - created pdu")
	pdu, err := nas_control.EncodeNasPduWithSecurity(ue, pdu, nas.SecurityHeaderTypeIntegrityProtectedAndCiphered, true, false)
	if err != nil {
		return nil, fmt.Errorf("Error encoding %s IMSI UE PduSession Release Request Msg", ue.UeSecurity.Supi)
	}
	log.Info("UL NAS Transport 3 - encoded pdu with security")
	return pdu, nil
}
...
func getUlNasTransport_PduSessionReleaseComplete(pduSessionId uint8, requestType uint8, dnnString string, sNssai *models.Snssai) (nasPdu []byte) {
	log.Info("getULNASTRansport Pdu session release complete")
